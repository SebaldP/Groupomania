...
console.log(req.body);
    ...
    resetPassword: (req, res, next) => {
        ...
        if (!Registration_REGEX.test(req.body.registration)) {
            return res.status(400).json({ alert: "Numéro de matricule incorrect !" });
        };
        if (!Passkey_REGEX.test(req.body.key)) {
            return res.status(401).json({ alert: "Minimum (8 caractères): 1 majuscule, 1 minuscule, 1 chiffre, 1 caractère spécial (!?@#%&*€¤)" });
        };
        next();
    },
    ...
    updateProfile: (req, res, next) => {
        ...
        if (Text_REGEX_A.test(req.body.pseudonym)) {
            return res.status(401).json({ alert: "Pas d'injection SQL dans l'entreprise ! Premier avertissement !" });
        };
        if (Text_REGEX_B.test(req.body.pseudonym)) {
            return res.status(401).json({ alert: "Pas d'injection SQL dans l'entreprise ! Deuxième avertissement !" });
        };
        if (Text_REGEX_C.test(req.body.pseudonym)) {
            return res.status(401).json({ alert: "Pas d'injection SQL dans l'entreprise ! Dernier avertissement !" });
        };
        next();
    },
